    ...
    return (
      <Html>
        <Head>
          ...
          (function(n, v, s, x, z) {
            x = window.GzRUM = {
                q: [],      // queue for holding early calls
                n: n,       // function name used globally (gzRum)
                v: v,       // version
                s: s,       // script source
                config: {   // static configuration block
            ...
            // define a global proxy function
            window[n] = function(c, p) {
                x.q.push({ c: c, p: p });   // enqueue early RUM commands
            };

            // dynamically load the actual RUM script
            z = document.createElement('script');
            z.async = true;
            z.src = s;
            document.head.insertBefore(z, document.head.getElementsByTagName('script')[0]);
            })(
            'gzRum',     // global function name
            '1.1.0',     // version
            'https://dev-genwiz-observability.com/RUMPerformanceMetrics.js' // remote script
            );
